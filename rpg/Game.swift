    ...
    func run() {
       ...
       for i in 1...totalPlayers {
        ...
        while activePlayers(allPlayers: players).count > 1  {
            if var currentPlayer:Player = players[currentPlayerIndex] , currentPlayer.isAlive {
                    ...
                    if let command = readLine(as: String.self) {
                        ...
                        if let move = availableMoves.first(where: { (move) -> Bool in
                            ...
                            case "seppuku":
                                ...
                                print("Вашият ход приключи.")
                            case "stats":
                                print("Health: \(currentPlayer.hero.lifePoitns)")
                                print("Energy: \(currentPlayer.hero.energy)")
                                print("Attack: \(currentPlayer.hero.armor!.attack + currentPlayer.hero.weapon!.attack)")
                                print("Defence: \(currentPlayer.hero.armor!.defence + currentPlayer.hero.weapon!.defence)")
                            default:
